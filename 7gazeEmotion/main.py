...
def interactive_startup() -> dict:
    ...
    while True:
        mode_input = input(
            "Input 0 to choose offline mode, input 1 to choose online mode: "
        ).strip()
        if mode_input == '0':
...
def main():
    ...
    # ── 交互式启动选择（仅在未通过命令行指定 --video/--camera 时触发）──────────
    if not args.config and not args.video and not args.image:
        ...
        if startup['use_camera']:
            # online 模式：使用摄像头
            pass  # use_camera 由 config 控制，video=None 即走摄像头分支
        else:
            # offline 模式：使用视频
...
if __name__ == '__main__':
    main()
    globalize()
    input("Press Enter to exit...")  # 加这行，报错后窗口不会关